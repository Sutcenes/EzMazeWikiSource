...
import com.mygdx.ezmazeFenetre.FenetreMenu;
...
public class WorldController extends InputAdapter {
	...
	private void collisionPersonnagecaseboue() {
		//dès qu'il arrive sur la caseboue on réduit sa vitesse pendant 3 secondes
		level.personnage.frottement.set(75,75);
	}
	...
	private void testCollision() {
		...
		for (Case c : level.caseboues) {
			r2.set(c.position.x,c.position.y,c.frontiere.width,c.frontiere.height);
			if(r1.overlaps(r2)) {
				onBoue = true;
				collisionPersonnagecaseboue();
			}
			if(!onBoue) {
